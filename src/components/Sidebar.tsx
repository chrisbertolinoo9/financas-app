...
export default function Sidebar({ view, setView, onImport }: Props) {
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 flex-col py-5 z-50 hidden md:flex" style={{ width: 232, background: 'var(--bg2)', borderRight: '1px solid var(--border)' }}>
      {/* Logo */}
      ...
      {/* Actions */}
      <div className="px-3.5 pt-3.5 pb-1 flex flex-col gap-2">
        <button onClick={() => {}} style={{ width:'100%', background:'linear-gradient(135deg,var(--accent),var(--accent2))', color:'#fff', border:'none', borderRadius:11, padding:'11px', fontFamily:'Sora,sans-serif', fontSize:12, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', gap:7, boxShadow:'0 4px 18px rgba(99,102,241,.28)' }}>
          <span>＋</span> Nova Transação
        </button>
        <button onClick={onImport} style={{ width:'100%', background:'rgba(6,182,212,.07)', color:'var(--cyan)', border:'1px solid rgba(6,182,212,.22)', borderRadius:11, padding:'10px', fontFamily:'Sora,sans-serif', fontSize:12, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:7 }}>
          📷 Importar por Imagem
        </button>
      </div>
      ...
      <div className="px-3.5 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
        <div className="flex items-center gap-2 p-2.5 rounded-xl mb-2" style={{ background: 'var(--bg3)' }}>
          ...
          <button onClick={logout} className="w-7 h-7 rounded-md flex items-center justify-center text-sm" style={{ background: 'rgba(239,68,68,.07)', border: '1px solid rgba(239,68,68,.13)', color: 'var(--muted)', cursor:'pointer' }} title="Sair">⏻</button>
        </div>
        <button onClick={() => { if(confirm('Apagar TODOS os dados?')) clearAll() }} style={{ width:'100%', padding:'7px', background:'transparent', border:'1px solid rgba(239,68,68,.15)', borderRadius:8, color:'var(--muted)', fontFamily:'Sora,sans-serif', fontSize:10, fontWeight:600, cursor:'pointer' }}>
